...
app.get("/health",async()=>({ok:true,service:"dansk-esim-api",version:"0.3.0"}));
...
app.get("/api/admin/orders",async(req,reply)=>{if(!adminAuthorized(String(req.headers.authorization??""),adminSecret??""))return reply.code(401).send({ok:false,error:"unauthorized"});return {ok:true,orders:[...db.ordersByUser("").concat([])]}});
